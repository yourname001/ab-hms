...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.rooms.update", $room->id) }}" enctype="multipart/form-data" autocomplete="off">
            @csrf
            @method('PUT')
                    ...
                    <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                            <div class="modal-body">
                                <div class="form-group">
                                    <img id="img" width="100%" class="img-thumbnail" style="border: none; background-color: transparent" src="{{ is_null($room->image) ? asset('images/image-icon.png') : asset('images/rooms/'.$room->image) }}" />
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('room_type') ? 'is-invalid' : '' }}" name="room_type_id" id="room_type_id" required>
                        @foreach($room_types as $id => $room_type)
                            <option value="{{ $id }}" @if(old('room_type_id')) {{ old('room_type_id') == $id ? 'selected' : '' }} @else {{ $room->room_type_id == $id ? 'selected' : '' }} @endif >{{ $room_type }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row">
                <div class="col-md-6">
                    ...
                    <input class="form-control {{ $errors->has('capacity') ? 'is-invalid' : '' }}" type="number" name="capacity" value="{{ old('capacity', $room->capacity) }}">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" name="amount" value="{{ old('amount', $room->amount) }}">
                ...
                <div class="col-md-6">
                    <div class="form-group form-check">
                        <input type="checkbox" class="form-check-input" id="featured" value="1" name="featured" @if(old('featured')) checked @endif>
                        <label class="form-check-label" for="featured">Featured</label>
                    </div>
                </div>
                <div class="col-md-6">
                    <label>Description</label>
                    <textarea class="form-control" name="description" cols="30" rows="10">{{ old('name', $room->description) }}</textarea>
                </div>
            </div>
            <div class="form-group">
